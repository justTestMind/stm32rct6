...
void OLED_Refresh(void)
{
	...
	for(i=0;i<8;i++)
	{
		...
		OLED_WR_Byte(0x10,OLED_CMD);   //设置高起始地址
		I2C_Start();
		Send_Byte(0x78);
		I2C_WaitAck();
		Send_Byte(0x40);
...
void OLED_ShowChinese(uint16_t x,uint16_t y,uint16_t num,uint16_t size1,uint16_t mode)
{
	uint16_t m,temp;
	uint16_t x0=x,y0=y;
	uint16_t i,size3=(size1/8+((size1%8)?1:0))*size1;  //得到字体一个字符对应点阵集所点的字数
	for(i=0;i<size3;i++)
	{
		if(size1==16)
				{temp=Hzk1[num][i];}//调用 16*16 字体
		else if(size1==24)
				{temp=Hzk2[num][i];}//调用 24*24 字体
		else if(size1==32)       
				{temp=Hzk3[num][i];}//调用 32*32 字体
		else if(size1==64)
				{temp=Hzk4[num][i];}//调用 64*64 字体
		else return;
		for(m=0;m<8;m++)
		{
			if(temp&0x01)OLED_DrawPoint(x,y,mode);
...
{
	uint16_t i,n,t=0,m=0,r;
	while(1)
	{
		if(m==0)
		{
	    OLED_ShowChinese(128,24,t,16,mode); //写入一个汉字保存在OLED_GRAM[][]中
			t++;
		}
		if(t==num)
			{
...
void OLED_Init(void)
{
    rcc_periph_clock_enable(OLED_SCL_RCC);
    rcc_periph_clock_enable(OLED_SDA_RCC);
    gpio_set_mode(OLED_SCL_GPIO, GPIO_MODE_OUTPUT_50_MHZ, GPIO_CNF_OUTPUT_OPENDRAIN, OLED_SCL_GPIO_PIN);
